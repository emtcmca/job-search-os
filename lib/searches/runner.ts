...
import {
  collectStructuredBoardCandidates,
  type BoardCandidate,
} from "@/lib/searches/boards";
...
export async function runSavedSearch(searchId: number) {
  ...
  const notes: string[] = [];
  const structuredBoardCandidates: BoardCandidate[] = [];
  ...
  for (const source of selectedSources) {
    ...
    const connection = connections.find(
      (item: PlatformConnectionRecord) => item.platform === source,
    );
    if (!connection) {
      notes.push(
  ...
  for (const trackedUrl of trackedUrls) {
    try {
      const boardCandidates = await collectStructuredBoardCandidates({
        trackedUrl,
        keywords,
        strictKeywordMatch: Boolean(filters.strictKeywordMatch),
      });

      if (boardCandidates && boardCandidates.length > 0) {
        discoveredPages += 1;
        for (const candidate of boardCandidates) {
          structuredBoardCandidates.push(candidate);
        }
        notes.push(
          `${trackedUrl} returned ${boardCandidates.length} structured ${boardCandidates[0]?.source ?? "board"} postings.`,
        );
        continue;
      }
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Unknown ATS board collector error.";
      notes.push(`${trackedUrl} board collection failed. ${message}`);
    }
  ...
  for (const candidate of structuredBoardCandidates) {
  ...
  for (const candidateUrl of candidateUrls) {
    if (importedBrowserUrls.has(candidateUrl)) {
      continue;
    }
  ...
  const successfulImports = imports.filter((result) => result.ok);
  const importedCount = successfulImports.length;
  const createdCount = successfulImports.filter((result) => result.createdNew).length;
  const mergedCount = successfulImports.filter((result) => result.dedupeMerged).length;
  const refreshedCount = successfulImports.filter(
    (result) => !result.createdNew && !result.dedupeMerged,
  ).length;
  ...
  const attemptedImports =
    structuredBoardCandidates.length + candidateUrls.size + browserImportCandidates.length;
  ...
  return {
    ok: true as const,
    message: `Scanned ${discoveredPages} tracked pages, attempted ${attemptedImports} imports, created ${createdCount} new jobs, refreshed ${refreshedCount} existing jobs, merged ${mergedCount} duplicate matches, and saw ${warningCount} warnings.`,
    importedCount,
    createdCount,
    refreshedCount,
    mergedCount,
...
export async function runActiveSavedSearches() {
  const searches = (await listSavedSearches()).filter((search) => search.isActive);
  const results = [];
